...
test.describe('太刀持ち従者の武器持ち替え省略＆リセット判定テスト', () => {
  ...
  test('太刀持ち従者あり：射撃後の接近戦武器への持ち替えラウンドが省略されること', async ({ page }) => {
    ...
    await page.locator('.archetype-card').nth(3).click();
    await page.locator('button:has-text("キャラクターの命運を紡ぎ出す")').click();
    await page.waitForSelector('.levelup-card', { state: 'visible', timeout: 5000 });

    // 3. レベルアップ・雇用画面
    // 太刀持ち従者を雇用
    await page.locator('.recruiter-column > div > div').filter({ hasText: '太刀持ち' }).locator('button:has-text("雇用")').click();
    await page.waitForTimeout(300);
    
    // 冒険開始
    await page.locator('button:has-text("冒険を開始する")').click();
    ...
    // 4. ダンジョン探索：d66を振る
    await page.locator('button:has-text("d66を振って次の部屋を探索する")').click();
    ...
    // 第0ラウンドで通常攻撃（射撃）を実行
    await page.locator('button:has-text("射撃攻撃")').click();
    await page.waitForTimeout(1000);

    // 接近戦へ移行するをクリック
    await page.locator('button:has-text("接近戦へ移行する")').click();
    await page.waitForTimeout(1000);
  ...
  test('戦闘をまたいだ射撃フラグのリセット判定：1戦目で射撃しても、2戦目で射撃していなければ持ち替えラウンドが発生しないこと', async ({ page }) => {
    ...
    if (await lootRollBtn.isVisible()) {
      await lootRollBtn.click();
      await page.waitForTimeout(1500);
    }

    // 戦闘終了
    await page.locator('button:has-text("結果を承認")').click();
    await page.waitForTimeout(1000);
