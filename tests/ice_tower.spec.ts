...
test('Ice Tower scenario full play-through verification', async ({ page }) => {
  ...
  console.log('Starting Ice Tower scenario play-through...');
  ...
  while (actionCount < maxActions && loopCount < maxLoops) {
    ...
    // 1. Scenario selection - Target "氷結の魔塔"
    if (await page.locator('.scenario-selector').isVisible({ timeout: 0 })) {
      const scenarioCard = page.locator('.scenario-card').filter({ hasText: '氷結の魔塔' }).first();
      if (await safeClick(scenarioCard, 'Selecting Ice Tower scenario card', 500)) {
        actionCount++;
      }
      continue;
    }
    ...
    if (await page.locator('.creator-card').isVisible({ timeout: 0 })) {
      ...
      await page.fill('#char-name', 'Ice Tower Hero');
