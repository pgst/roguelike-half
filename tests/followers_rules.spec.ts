...
test.describe('従者魔術師（Mage）の呪文カスタマイズ＆戦闘行使テスト', () => {
  ...
  test('魔術師従者：氷槍を習得して雇用し、戦闘で氷槍（2点ダメージ）を唱えること', async ({ page }) => {
    ...
    await mageRecruiter.locator('select').first().selectOption('氷槍');
    await page.waitForTimeout(200);

    // 武器属性を「斬撃」にする
    await mageRecruiter.locator('input[value="slash"]').click();
    await page.waitForTimeout(200);

    // 魔術師を雇用
    await mageRecruiter.locator('button:has-text("雇用")').click();
