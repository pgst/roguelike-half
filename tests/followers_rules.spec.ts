...
test.describe('従者魔術師（Mage）の呪文カスタマイズ＆戦闘行使テスト', () => {
  ...
  test('魔術師従者：炎球を習得して雇用し、戦闘で炎球（地形範囲攻撃）を唱えること', async ({ page }) => {
    ...
    const logbook = page.locator('.logbook-entries');
    await expect(logbook).toContainText('炎球', { timeout: 5000 });
    await expect(logbook).toContainText('狭い場所', { timeout: 5000 });
    await expect(logbook).toContainText('炎球が炸裂', { timeout: 5000 });
  });
